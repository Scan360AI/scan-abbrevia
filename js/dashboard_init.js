            ...
            const trendPfnEbitdaOptions = {
                 ...
                 scales: {
                     y: { // Scala per il rapporto
                         ...
                         suggestedMin: -1, // Modificato per ABBREVIA SPA (cassa netta in alcuni periodi)
                         suggestedMax: 3, // Adattato per ABBREVIA SPA
                             ...
                             label: function(context) {
                                 ...
                                 if (label) label += ': ';
                                  if (context.parsed.y !== null) {
                                     ...
                                     // Aggiunta valutazione per ABBREVIA SPA
                                     if (context.dataset.label === 'PFN/EBITDA') {
                                         const value = context.parsed.y;
                                         if (value <= 0) label += ' (Cassa netta)';
                                         else if (value <= 1.0) label += ' (Eccellente)';
                                         else if (value <= 2.0) label += ' (Ottimo)';
                                         else if (value <= 3.0) label += ' (Buono)';
                                         else label += ' (Attenzione)';
                                     }
                                  } else {
                                      label += 'N/D';
                                  }
